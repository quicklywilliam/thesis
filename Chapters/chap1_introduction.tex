...
However, the discovery of the $\lambda$-calculus did not end the search for new algebra.  
...
For example, we might make a conversation on a phone somewhere in our network a process. Since \inidx{concurrency} of processes is such a basic operation, we make it a part of our algebra.  
A system, then, is simply a group of processes which are executing concurrently. An important thing about processes is that they maintain computation state independently of one another.  
...
As with functions in the $\lambda$-calculus, processes are the basic unit of a program in the \picalc. Any bit of functionality can be referred to as a process, with no specification of the granularity.
...
Hence, for example, $Mobile(t,s)$ would be this right-hand term with the channels $t$ and $s$ substituted for $talk$ and $switch$. We call $Mobile(t,s)$ the \defmargin{interface} of the right-hand term.
