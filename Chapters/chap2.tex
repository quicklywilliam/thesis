...
To see how, consider the following system:
	\begin{align}
		...
		Sys_1 & \pdef \new{d}(F_1\angle d \rangle \comp F_2\langle d\rangle)
	\end{align}
...
When a process has free channel names occurring in its body, as is the case with $a$, we will place those names inside the parenthesis in the process's interface.  
...
\[
	\new{b,c}Q_1(b) \comp Q_2(b,c)
\]
Process components $Q_1$, $Q_2$ both expose the same channel $b$ which the system has bound.  
...
\end{align}
You can probably see that $c$'s scope is extruded by sending over $d$ and that the left side of the term will then use $c$ to communicate with the right side.  
Now we will show that this happens using the reduction rules.  
...
That is, a process $P$ can be said to be capable of the transition $P \evolves{\receives{c}{\tuple{X}}} Q$, which is to say that $P$ can receive $\tuple{X}$ along $c$ resulting in the residual $Q$.  
The capability of a process to evolve in this way is given by the rule (A-IN) in \reffig{apiactionrules}, which say that for a general process $c$ and input $\tuple{V}$
\[
...
That is, $\tuple{V}$ and $\tuple{X}$ must have the same arity\refmargin{arity}and in a typed system we'd want to ensure that their types were compatible
...
We denote the set of names that are bound\refmargin{bound identifiers} in the send action by $\exports{\tuple{B}}$, and we say that this set of names is \defmargin{exported} by the process.  
...
\]
The capability for $\tau$ is defined through the rules (A-COMM), (A-REP), (A-EQ) and (A-NEQ).  (A-REP), (A-EQ) and (A-NEQ) simply the provide the same interval evolution capabilities as (R-REP), (R-EQ) and (R-NEQ) do in the \hyperref[secreducationsemantics]{reduction semantics}. (A-COMM), however, is more subtle then (R-COMM) and it makes sense to look at a few of the other rules of \reffig{apiactionrules} before characterizing it.

Hence, we now formally define the action relation under the following rules (with their preconditions listed alongside them):
...
\[
	P\comp Q \evolves{\tau} \new{\tuple{B}}(P'\comp Q'),
\]
no external contributions are needed for evolution.  
The exported names $\exports{\tuple{B}}$ are now scopes to both processes as a result of the original export by $Q$.  
...
\begin{center}\begin{tabular}{lllr}
	...
	& $\evolves{\exports{ack}\sends{c}{ack}}$ & $\new{ack}(stop \comp \receive{ack}{}(R \comp F_1\langle c\rangle))$ & \tiny{(A-OPEN, A-OUT)}\\
	& $\evolves{\exports{\receives{ack}{}}}$ & $\new{ack}(stop \comp (R \comp F_1\langle c\rangle)) $& \tiny{(A-IN)}\\
\end{tabular}\end{center}
...
Our cell might have channels for getting and setting its value, but as many processes as required could get the cell's value without it changing or disappearing.  
It turns out that we can create this cell just using message passing.  
This section will explore the creation and use of a memory cell, serving as a good review and usage example of the concepts introduced in the rest of the chapter.
...
The following line has a recursive listener process that, when given a channel via $get$, gets the value from $c$ and sends it back via the supplied channel $x$.  
...
The old value is pulled from $c$ and simply discarded (ie not used), whereas the new value $b$ is pushed to the $c$ this time.  
...
In the following system we assume that we have access to standard arithmetic operations and that memory cells are initialized with the integer 0.  
...
\]	
	To see why this system produces the behavior we expect, we substitute in $Cell$'s definition and observe scope restrictors for $a$ and $a_2$ can be moved out using $(S-REST-COMP)$:
	\begin{equation}\begin{split}
		\new{get_1, set_1a,a_2}&(\new{c} (\send{c}{init} \\
			&\comp\rec{g}(\receive{get}{x}\receive{c}{y}(\send{x}{y}\comp \send{c}{y}\comp g))\\
			&\comp\rec{s}(\receive{set}{x,b}\receive{c}{y}(\send{x}{b}\comp\send{c}{b}\comp s)))\\
...
\begin{equation}\begin{split}
	\new{get_1, set_1a,a_2}&(\new{c} (\send{c}{init} \\
		&\comp \receive{get}{x}\receive{c}{y}(\send{x}{y}\comp \send{c}{y} \\
		&\comp\rec{g}(\receive{get}{x}\receive{c}{y}(\send{x}{y}\comp \send{c}{y}\comp g))\\
		&\comp\rec{s}(\receive{set}{x,b}\receive{c}{y}(\send{x}{b}\comp\send{c}{b}\comp s)))\\
		&\comp \send{get_1}{a}\\
		&\comp \receive{a}{y} \send{set_1}{y+1,a_2} \comp \receive{a_2}{z}\send{o}{z})
\end{split}\end{equation}
Now we can apply (R-COMM) to the memory cell's getter...
\begin{equation}\begin{split}
	\new{get_1, set_1a,a_2}&(\new{c} (\send{c}{init} \\
		...
		&\comp \receive{a}{y} \send{set_1}{y+1,a_2} \comp \receive{a_2}{z}\send{o}{z})
\end{split}\end{equation}
...and apply it once again to the memory's `internal' channel $c$, stripping out the initializer:
\begin{equation}\begin{split}
	\new{get_1, set_1a,a_2}&(\new{c} (\\
		...
		&\comp \receive{a}{y} \send{set_1}{y+1,a_2} \comp \receive{a_2}{z}\send{o}{z})
\end{split}\end{equation}
Which, with one more (R-COMM), finally sends back the $init$ value to the $MemoryClient$.  
...
This is due to a \defmargin{race condition}, a classic issue in concurrent systems, which means that the output of a system is dependent on the timing of computations in the system.  
...
In that case, $MemoryClient_1$ will have `stored' the value 0 for the cell in $y$ that is no longer current.  
...
Asynchronous channels map naturally to this type of behavior, as in we will see in our $MemoryClient$, which we've modified to include a lock.  

We use the short hand $l?lock.P$ to mean:
\[
	\rec{p}(\receive{l}{x} \pif{x=true}\ then (P\comp \send{l}{true}) else (p \comp \send{l}{true}))
\]
Here we check the lock channel $l$ and execute $P$ if it is currently set to $false$. 
Otherwise we recursively try to get the lock again.
Notice that whether the lock is already set to $true$ or we are able to acquire it, we need to send $true$ to $l$ so that another process can't acquire it.
...
\begin{equation}\begin{split}
	\evolves{\tau} & \receive{l}{x} \pif{x=true}\ then (P\comp \send{l}{true}) else \\
	&l?lock.P \comp \send{l}{true}))
\end{split}\end{equation}
If $\receive{l}{x}$ receives a true, then
\begin{equation}\begin{split}
	\evolves{\receives{c}{true}} & \pif{true=true}\ then (P\comp \send{l}{true}) else \\
	&l?lock.P \comp \send{l}{true}))
\end{split}\end{equation}
We can now apply (A-EQ), yielding:
\begin{equation}\begin{split}
	\evolves{\tau} & P\comp \send{l}{true}\\
\end{split}\end{equation}
As expected.  Otherwise, if $\receive{l}{x}$ receives a false, then $l?lock.P$
\begin{equation}\begin{split}
	\evolves{\receives{c}{false}} & \pif{false=true}\ then (P\comp \send{l}{true}) else \\
	&l?lock.P \comp \send{l}{true}))
\end{split}\end{equation}
This time using (A-NEQ),
\begin{equation}\begin{split}
	\evolves{\tau} &l?lock.P \comp \send{l}{true}))
\end{split}\end{equation}
Which of course is just the lock after using (A-OUT) to express the capability of its sending $true$ back to the lock.  Thus, we have verified that the lock works as expected.\todo{line up all the above equations by putting them in one array and using breaks}

Now, unlocking is done via the shorthand $l!unlock$, meaning
\[
	\receive{l}{x} \send{l}{false}
\]
Here we simply discard the current $l$ value and set it to $false$.  
...
\end{split}\end{equation}
Using action semantics to verify that the whole system works as intended is somewhat tedious but perhaps instructive and will thus be left as an exercise to the reader.  We will return to the subject of locks in Chapter \ref{sync_and_dist_sys}, where they will play a surprisingly similar in the implementation of a synchronous \picalc.