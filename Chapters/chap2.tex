...
Consider the following system:
	\begin{equation}\begin{split}
		Cell(get,set) \pdef \new{c} & (\send{c}{init} \\
		& \comp\rec{g}(\receive{get}{b}\receive{c}{y}(\send{b}{y}\comp \send{c}{y}\comp g))\\
		&\comp\rec{s}(\receive{set}{x,b}\receive{c}{y}(\send{b}{x}\comp\send{c}{x}\comp s)))
	\end{split}\end{equation}
	In $Cell$, we first create a new channel $c$ that will be used to `store' the value.  
We then initialize $c$ with the value $init$.  
The following line has a recursive listener process that, when given a channel via $get$, gets the value from $c$ and sends it back via the supplied channel.  
...
It is also sent via $b$ as an acknowledgment that the cell's new value has been set.
