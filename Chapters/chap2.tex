...
Finally, we conclude the chapter with an extended example that provides an interesting \picalc\ implementation while demonstrating its syntax and rules.
	\section{Syntax}\label{spisyntax}
	The terms of the \picalc\ operate on a space of \defmargin{identifiers} which consists of names $a,b,c,...,n,m,o$ for communication channels, variables $v,w,x$ which can refer to channels, and recursive variables $p,q,r$, explained in more detail below.  
...
The term \pstop\ describes a terminating process: one that simply halts.\index{termination}
...
In that case, $c$ is `scoped' to $P$ and $R$.  
...
For example, in the following term, the value received on $c$ is checked; if it matches $a$ then a handshake is sent along $a$ (which is referred to by $x$), otherwise the process terminates.
	...
	Recursion is built into the language using the syntax $\rec{p}R$.\index{recursion}
The process $\rec{p}R$ itself is referred to by the variable $p$, which is used somewhere in $R$ to express a recursive call.  
...
It then sends a handshake response on $x_1$, while another responder process is run in parallel.
	...
	\end{align}
The scope of the variable $p$ is restricted to $R$, just as $\tuple{X}$ is restricted to $R$ in $\receive{c}{\tuple{X}}R$ and $n$ is restricted to $R$ in $\new{n}R$.
	
The composition operator$\comp$should be seen as having lower grouping priority than the other operators.  Thus, for example, $\new{n}R \comp Q$ should be read as $(\new{n}R) \comp Q$.  Operators that join using the dot (.) notation should always be taken as being grouped together first.  Thus, $\new{c}\receive{c}{x}\send{a}{b}$ should be read as $\new{c}(\receive{x}{x}\send{a}{b})$ and not $(\new{c}\receive{x}{x})(\send{a}{b})$.
...
\begin{example}{exsummation}
	 Many presentations (including that of the next chapter) of the \picalc\ involve a \inidx{choice} or \inidx{summation} operator as in $P+Q$ for two process terms $P$ and $Q$.  
The meaning is essentially that either $P$ or $Q$ can be non-deterministically executed, and the other process will not be.  
However, we can have non-deterministic behavior without using the choice operator:
	\begin{align}
		\send{c}{} \comp \receive{c}{}P \comp \receive{c}{}Q
	\end{align}
	In the above process, either $Q$ or $P$ (but not both) will be executed.  
Sending an empty signal along $c$, we cannot control which process consumes the signal. 
...
Our asynchronous calculus can model \inidx{synchronous} sending by using a private channel that acknowledges when a value is received.\index{acknowledgement channels}  
...
Following this, we will develop the notion of \emph{contexts} and then use it to build an equivalence relation among processes.\index{structural equivalence}
...
Third, the recursive variable $x$ is bound within $R$ in the recursive expression $\rec{x}R$, bound particularly to the whole process itself.
...
But the received term $n$ is not the same as the $n$ occurring in $P(a)$ --- it was a free channel sent by the process $\send{a}{n}$ outside the scope restriction $\new{n}$  Thus, the problem in performing $\subst{n}{x}$ is that it would have the free name $n$ from the outside process bound as if it were the same $n$ bound by $P(a)$.  
...
We say that a process $Q$ with interface $Q(\tuple{V})$ \defmarginwide[exposes]{exposed channels} the names $\tuple{V}$.  
...
\begin{definition}{Context}
	A context $\mathbb{C}$ is given by:\index{contexts}
	\[
...
The first and second state that composition is commutative and associative.  
Thus, we will omit parentheses around compositions when the meaning is clear.  
(S-COMP-ID) states that a terminated process can be eliminated from a composition.  
(S-REST-ID) states that a channel scope restriction operator can be eliminated when its scope is only over a terminated process. (S-REST-COMM) states that scope ordering does not matter, justifying our shorthand $\new{c,d} P$.  
...
The set of rules given below show how a process can internally evolve through a number of computation steps.\index{internal evolution}
...
\end{align}
	...
	To find this $Q$, we can apply (R-COMM), which we can do inside of the restriction operator thanks to the reduction relation's\refmargin{contextual} contextuality, and apply a substitution to (\ref{exscopeextr_eqn2}), resulting in:
\begin{align}
...
\end{definition}\index{labelled transition systems}
...
That is, a process $P$ is said to be capable of making the transition $P \evolves{\receives{c}{\tuple{V}}} Q$, which is to say that $P$ can receive $\tuple{V}$ along $c$ to become the residual process $Q$.  
...
\begin{definition}{Action}\label{apiactionrules}
	...
	\begin{center}\begin{tabular}{rllll}
		...
		\multicolumn{3}{c}{$\underline{P \evolves{\alpha} P'}$} & \multirow{2}{*}{\footnotesize{$\textstyle \mbox{\emph{bn}}(\alpha) \cap \mbox{\emph{fn}}(Q) = \emptyset$ }} & \multirow{2}{*}{\tiny{(A-COMP)}}\\
	...
	\end{tabular}\end{center}
\end{definition}
In the preconditions above, we have subtly extended our notion of bound names to the case of actions.  
...
Since there are no names (only variables) in a receive expression, $bn(\receives{c}{\tuple{V}}) = \emptyset$.  
Thus, the only names that are bound in an action are those exported by a send action, in which case $bn(\exports{\tuple{B}}\sends{c}{\tuple{V}}) = \tuple{B}$.

Together, (A-COMP) and (A-REST) provide a capability similar to \refmargin{contextual}contextuality.  
...
The key difference between these rules and contextuality is that here we need to be careful about inadvertently capturing bound variables.
...
\begin{center}\begin{tabular}{rllll}
	\multicolumn{3}{c}{$\underline{\receive{c}{}\pstop \evolves{\receives{c}{}} \pstop}$} & & \multirow{2}{*}{\tiny{A Corrupt (A-REST)}}\\
...
\begin{center}\begin{tabular}{rllll}
	\multicolumn{3}{c}{$\underline{\new{b}\send{c}{b}\evolves{\exports{b}\sends{c}{x}} \pstop}$} & & \multirow{2}{*}{\tiny{A Corrupt (A-COMP)}}\\
...
Note that $n$ cannot be $c$ since restricting $n$ (i.e.,~$c$) to $P$ would cause a scoping issue that interfered with an outside process's ability to receive on $c$.
...
Suppose we have that $P \evolves{\receives{c}{\tuple{V}}} P'$ and $Q \evolves{\exports{\tuple{B}}\sends{c}{\tuple{V}}} Q'$.
Then according to (A-COMM), if we compose the two, forming $P\comp Q$, then the simultaneous (compositionally) occurrence of an input action $\receives{c}{\tuple{V}}$ and matching output action $\exports{\tuple{B}}\sends{c}{\tuple{V}}$ can be replaced with $\tau$.
That is, we can say:
\[
	P\comp Q \evolves{\tau} \new{\tuple{B}}(P'\comp Q'),
\]
...
\[
	F_1(c) \pdef \rec{z} (\new{ack}(\send{c}{ack} \comp \receive{ack}{}(R \comp z)))
\]
We then went on to show how this process might work as a component in an example system.  
But suppose we wanted to characterize this process's behavior in general.  
We could use reduction semantics to describe how it it can evolve internally (in this case there isn't much we can do except unwind the recursion), but we'd also want to include a description of how that process behaves externally.  
...
\end{itemize}
Thus, we have completely characterized $F_1(c)$'s capabilities to act as a recursive synchronous output process in a larger system --- and we have done so without having to describe the larger system. 
\end{example}
...
Passing a new value on the $set$ channel would result in the cell changing its value to the value supplied.
...
It turns out that we can create this cell just using message passing.  
	...
	The problem that we have to work around is that receiving an identifier from a channel removes that value from the channel.  
...
The old value is pulled from $c$ and simply discarded.  
...
Now let us look at how this memory cell might be used.
...
\]	
	To see why this system produces the behavior we expect, we substitute the definitions for $Cell$ and \emph{MemoryClient} into the system and observe scope restrictors for $a$ and $a_2$ can be moved out using (S-REST-COMP):
	\begin{equation}\begin{split}
...
\begin{equation}\begin{split}
	\new{get_1, set_1,a,a_2}&(\new{c} (\\
		...
		&\comp \receive{a}{y} \send{set_1}{y+1,a_2} \comp \receive{a_2}{z}\send{o}{z})
\end{split}\end{equation}
Which, with one more (R-COMM), finally sends back the $init$ value to the \emph{MemoryClient} on $a$.  
...
\end{split}\end{equation}
If we were to run these as such, we may find that the output channel was producing some unexpected results.  
...
The usual solution to this type of problem is by implementing a \defmargin[lock]{locks}.  
...
Asynchronous channels map naturally to this type of behavior, as we will see in our \emph{MemoryClient}, which we can modify to use a lock.  
...
Our \emph{MemoryClient} thus becomes:
\begin{equation}\begin{split}
